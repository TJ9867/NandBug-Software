...
class NandFSM(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.FSM() as fsm:
            ...
            with m.State("IDLE"):
                with m.If(self.ryby == 1):  # Make sure the NAND Flash is ready
                    ...
                    with m.Elif(self.read):
                        m.d.sync += [self.cle.o.eq(0),
                                     self.ale.o.eq(0)]
                        m.next = "READ"

                    with m.Else():
                        m.d.sync += [self.cle.o.eq(0), self.ale.o.eq(0)]
                ...
                m.d.sync += [self.we.o.eq(1), self.re.o.eq(1)]
            ...
            with m.State("WRITE"):
                ...
                with m.Switch(write_type):
                    with m.Case(WriteType.CMD):
                        m.d.sync += self.cle.o.eq(1)
                    with m.Case(WriteType.ADDR):
                        m.d.sync += self.ale.o.eq(1)

                m.next = "WRITE_HOLD"

            with m.State("WRITE_HOLD"):
                m.d.sync += [self.we.o.eq(1)]
                m.next = "IDLE"

            with m.State("READ"):
                m.d.sync += [self.re.o.eq(0),
                             self.io_oe.eq(0)]
                m.next = "READ_SAMPLE"
